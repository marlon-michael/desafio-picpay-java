...
public class AccountRepository implements AccountGateway {
    ...
    public List<Account> findAll() {
        ...
        String json = redis.opsForValue().get("accounts");
        ...
        accountsFromPersistence = jpaAccountRepository.findAll();
        accounts = accountsFromPersistence
            .stream()
            .map(entity -> AccountMapper.fromEntityToDomain(entity))
            .toList();
        ...
        try {
            redis.opsForValue().set("accounts", objectMapper.writeValueAsString(accountsFromPersistence));
        } catch (Exception exception) {
            System.out.println(exception.getMessage());
        }
